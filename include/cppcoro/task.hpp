...
class task_promise_base {
    ...
    struct final_awaitable {
        ...
        await_suspend(cppcoro::coroutine_handle<PROMISE> coroutine) noexcept {
...
template <typename T> class task_promise final : public task_promise_base {
  public:
    ...
    using rvalue_type =
        std::conditional_t<std::is_arithmetic_v<T> || std::is_pointer_v<T>, T,
                           T &&>;
...
template <typename T = void> class [[nodiscard]] task {
  ...
  private:
    struct awaitable_base {
        ...
        awaitable_base(
            cppcoro::coroutine_handle<promise_type> coroutine) noexcept
        ...
        bool
        await_suspend(cppcoro::coroutine_handle<> awaitingCoroutine) noexcept {
            // NOTE: We are using the bool-returning version of await_suspend()
            // here to work around a potential stack-overflow issue if a
            // coroutine awaits many synchronously-completing tasks in a loop.
            //
            // We first start the task by calling resume() and then
            // conditionally attach the continuation if it has not already
            // completed. This allows us to immediately resume the awaiting
            // coroutine without increasing the stack depth, avoiding the
            // stack-overflow problem. However, it has the down-side of
            // requiring a std::atomic to arbitrate the race between the
            // coroutine potentially completing on another thread concurrently
            // with registering the continuation on this thread.
            //
            // We can eliminate the use of the std::atomic once we have access
            // to coroutine_handle-returning await_suspend() on both MSVC and
            // Clang as this will provide ability to suspend the awaiting
            // coroutine and resume another coroutine with a guaranteed
            // tail-call to resume().
            m_coroutine.resume();
            return m_coroutine.promise().try_set_continuation(
                awaitingCoroutine);
        }
#endif
    };
...
template <typename T> task<T> task_promise<T>::get_return_object() noexcept {
    return task<T>{
        cppcoro::coroutine_handle<task_promise>::from_promise(*this)};
}
