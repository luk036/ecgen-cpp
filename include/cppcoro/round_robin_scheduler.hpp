...
template <size_t N> class round_robin_scheduler {
    static_assert(N >= 2, "Round robin scheduler must be configured to support "
                          "at least two coroutines");
  ...
  public:
    round_robin_scheduler() noexcept
        : m_index(0), m_noop(cppcoro::noop_coroutine()) {
    ...
    ~round_robin_scheduler() {
        ...
        assert(std::all_of(m_coroutines.begin(), m_coroutines.end(),
  ...
  private:
    cppcoro::coroutine_handle
    exchange_next(cppcoro::coroutine_handle<> coroutine) noexcept {
        auto coroutineToResume = std::exchange(
            m_scheduler.m_coroutines[m_scheduler.m_index], awaitingCoroutine);
        m_scheduler.m_index =
            m_scheduler.m_index < (N - 2) ? m_scheduler.m_index + 1 : 0;
