...
namespace cppcoro {
    ...
    template <size_t N> class round_robin_scheduler {
        static_assert(N >= 2,
                      "Round robin scheduler must be configured to support "
      ...
      public:
        round_robin_scheduler() noexcept : m_index(0), m_noop(cppcoro::noop_coroutine()) {
      ...
      private:
        cppcoro::coroutine_handle exchange_next(cppcoro::coroutine_handle<> coroutine) noexcept {
