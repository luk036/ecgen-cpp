...
namespace cppcoro {
    ...
    class single_consumer_event {
      public:
        ...
        ///
        /// If the event is already set then the awaiting coroutine will not be
        /// suspended and will continue execution. If the event was not yet set then
        /// the coroutine will be suspended and will be later resumed inside a
        /// subsequent call to set() on the thread that calls set().
              ...
              public:
                ...
                bool await_suspend(cppcoro::coroutine_handle<> awaiter) {
                    ...
                    return m_event.m_state.compare_exchange_strong(
                        oldState, state::not_set_consumer_waiting, std::memory_order_release,
