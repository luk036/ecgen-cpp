...
template <typename FUNC, typename AWAITABLE> class fmap_awaiter {
  ...
  public:
    ...
    decltype(auto)
    await_suspend(cppcoro::coroutine_handle<PROMISE> coro) noexcept(noexcept(
        static_cast<awaiter_t &&>(m_awaiter).await_suspend(std::move(coro)))) {
...
template <typename FUNC> struct fmap_transform {
    explicit fmap_transform(FUNC &&f) noexcept(
