...
public:
  ...
  public:
    ...
    explicit constexpr const_iterator(SEQUENCE value) noexcept
        : m_value(value) {}

    const SEQUENCE &operator*() const noexcept { return m_value; }
    const SEQUENCE *operator->() const noexcept {
  ...
  constexpr sequence_range first(size_type count) const noexcept {
    return sequence_range{
        m_begin, static_cast<SEQUENCE>(m_begin + std::min(size(), count))};
  }
