...
class shared_task_promise_base {
    ...
    struct final_awaiter {
        ...
        void await_suspend(cppcoro::coroutine_handle<PROMISE> h) noexcept {
            ...
            void *waiters = promise.m_waiters.exchange(
                valueReadyValue, std::memory_order_acq_rel);
            if (waiters != nullptr) {
                shared_task_waiter *waiter =
                    static_cast<shared_task_waiter *>(waiters);
                ...
                // Resume last waiter in tail position to allow it to
                // potentially be compiled as a tail-call.
                waiter->m_continuation.resume();
            }
        }
  ...
  public:
    shared_task_promise_base() noexcept
        : m_refCount(1), m_waiters(&this->m_waiters), m_exception(nullptr) {}
    ...
    bool try_await(shared_task_waiter *waiter,
        ...
        constexpr void *startedNoWaitersValue =
...
template <> class shared_task_promise<void> : public shared_task_promise_base {
  public:
    shared_task_promise() noexcept = default;
...
template <typename T> void swap(shared_task<T> &a, shared_task<T> &b) noexcept {
    a.swap(b);
}
