...
class io_service {
  public:
    ...
    [[nodiscard]] timed_schedule_operation
    schedule_after(const std::chrono::duration<REP, PERIOD> &delay,
    ...
    /// are no events pending or the io_service is stopped then return
    /// immediately.
    ///
    /// \return
    /// The number of events processed during this call.
    ...
    ///
    /// This will cause any threads currently in a call to one of the
    /// process_xxx() methods to return from that call once they finish
    /// processing the current event.
    ///
    /// This call does not wait until all threads have exited the event loop so
    /// you must use other synchronisation mechanisms to wait for those threads.
    void stop() noexcept;
    ...
    /// to process events.
    ///
    /// After calling stop() you should ensure that all threads have returned
    /// from calls to process_xxx() methods before calling reset().
    void reset();
...
class io_service::timed_schedule_operation {
  public:
    ...
    timed_schedule_operation &
    operator=(timed_schedule_operation &&other) = delete;
    timed_schedule_operation(const timed_schedule_operation &other) = delete;
    timed_schedule_operation &
    operator=(const timed_schedule_operation &other) = delete;
  ...
  private:
    ...
    std::optional<cppcoro::cancellation_registration>
