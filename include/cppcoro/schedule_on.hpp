...
decltype(auto) operator|(T &&value,
                         schedule_on_transform<SCHEDULER> transform) {
...
template <typename T, typename SCHEDULER>
async_generator<T> schedule_on(SCHEDULER &scheduler,
  ...
  // below. This ensures that all calls to the generator's
  // coroutine_handle<>::resume() are executed on the execution context of the
  // scheduler.
  co_await scheduler.schedule();
