...
class socket {
  public:
    ...
    ///
    /// \param ioSvc
    /// The I/O service the socket will use for dispatching I/O completion
    /// events.
    ///
    /// \return
    ...
    /// reset. The destructor will not block waiting for queueud data to be
    /// sent. If you need to ensure that queued data is delivered then you must
    ...
    cppcoro::detail::win32::socket_t native_handle() noexcept {
        return m_handle;
    }

    /// Query whether I/O operations that complete synchronously will skip
    /// posting an I/O completion event to the I/O completion port.
    ///
    /// The operation class implementations can use this to determine whether or
    /// not it should immediately resume the coroutine on the current thread
    /// upon an operation completing synchronously or whether it should suspend
    /// the coroutine and wait until the I/O completion event is dispatched to
    /// an I/O thread.
    bool skip_completion_on_success() noexcept {
        return m_skipCompletionOnSuccess;
    }
#endif
    ...
    /// This can be either an unspecified address (in which case it binds to all
    /// available interfaces) and/or an unspecified port (in which case a random
    /// port is allocated).
    ///
    /// \throws std::system_error
    ...
    /// call to 'accept()'.
    ///
    /// The backlog of connections ready to be accepted will be set to some
    /// default suitable large value, depending on the network provider. If you
    /// need more control over the size of the queue then use the overload of
    /// listen() that accepts a 'backlog' parameter.
    ///
    /// \throws std::system_error
    ...
    ///
    /// \param backlog
    /// The maximum number of pending connections to allow in the queue of
    ...
    [[nodiscard]] socket_connect_operation
    connect(const ip_endpoint &remoteEndPoint) noexcept;
    ...
    /// cancelled then it will complete by throwing a
    /// cppcoro::operation_cancelled exception.
    ///
    /// \return
    /// An awaitable object that will start the connect operation when
    /// co_await'ed and will suspend the coroutine, resuming it when the
    /// operation completes. The result of the co_await expression has type
    /// 'void'.
    [[nodiscard]] socket_connect_operation_cancellable
    connect(const ip_endpoint &remoteEndPoint, cancellation_token ct) noexcept;

    [[nodiscard]] socket_accept_operation
    accept(socket &acceptingSocket) noexcept;
    [[nodiscard]] socket_accept_operation_cancellable
    accept(socket &acceptingSocket, cancellation_token ct) noexcept;

    [[nodiscard]] socket_disconnect_operation disconnect() noexcept;
    [[nodiscard]] socket_disconnect_operation_cancellable
    disconnect(cancellation_token ct) noexcept;

    [[nodiscard]] socket_send_operation send(const void *buffer,
                                             std::size_t size) noexcept;
    [[nodiscard]] socket_send_operation_cancellable
    send(const void *buffer, std::size_t size, cancellation_token ct) noexcept;

    [[nodiscard]] socket_recv_operation recv(void *buffer,
                                             std::size_t size) noexcept;
    [[nodiscard]] socket_recv_operation_cancellable
    recv(void *buffer, std::size_t size, cancellation_token ct) noexcept;

    [[nodiscard]] socket_recv_from_operation
    recv_from(void *buffer, std::size_t size) noexcept;
    [[nodiscard]] socket_recv_from_operation_cancellable
    recv_from(void *buffer, std::size_t size, cancellation_token ct) noexcept;

    [[nodiscard]] socket_send_to_operation
    send_to(const ip_endpoint &destination, const void *buffer,
            std::size_t size) noexcept;
    [[nodiscard]] socket_send_to_operation_cancellable
    send_to(const ip_endpoint &destination, const void *buffer,
            std::size_t size, cancellation_token ct) noexcept;
...
#if CPPCORO_OS_WINNT
    explicit socket(cppcoro::detail::win32::socket_t handle,
