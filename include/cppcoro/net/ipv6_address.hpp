...
public:
  ...
  constexpr ipv6_address(std::uint16_t part0, std::uint16_t part1,
  ...
  /// by separated by a ':'. The longest contiguous run of zero parts is
  /// contracted to "::".
  ///
  /// For example:
...
constexpr ipv6_address::ipv6_address(std::uint16_t part0, std::uint16_t part1,
                                     ...
                                     std::uint16_t part4, std::uint16_t part5,
                                     std::uint16_t part6, std::uint16_t part7)
    : m_bytes{static_cast<std::uint8_t>(part0 >> 8),
...
constexpr ipv6_address::ipv6_address(const std::uint16_t (&parts)[8])
    : ipv6_address(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5],
                   parts[6], parts[7]) {}
