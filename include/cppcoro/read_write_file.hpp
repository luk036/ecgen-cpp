...
class read_write_file : public readable_file, public writable_file {
  public:
    ...
    ///
    /// \param shareMode
    /// Specifies the access to be allowed on the file concurrently with this
    /// file access.
    ///
    /// \param bufferingMode
    /// Specifies the modes/hints to provide to the OS that affects the
    /// behaviour of its file buffering.
    ///
    /// \return
    ...
    [[nodiscard]] static read_write_file
    open(io_service &ioService, const cppcoro::filesystem::path &path,
