...
template <typename RESULT> class sync_wait_task_promise final {
  using coroutine_handle_t =
      cppcoro::coroutine_handle<sync_wait_task_promise<RESULT>>;
...
template <> class sync_wait_task_promise<void> {
  using coroutine_handle_t =
      cppcoro::coroutine_handle<sync_wait_task_promise<void>>;
...
#if CPPCORO_COMPILER_MSVC && CPPCORO_COMPILER_MSVC < 19'20'00000
// HACK: Work around bug in MSVC where passing a parameter by universal
// reference results in an error when passed a move-only type, complaining that
// the copy-constructor has been deleted. The parameter should be passed by
// reference and the compiler should notcalling the copy-constructor for the
// argument
template <typename AWAITABLE,
          typename RESULT =
...
sync_wait_task<RESULT> make_sync_wait_task(AWAITABLE &awaitable) {
  // HACK: Workaround another bug in MSVC where the expression 'co_yield
  // co_await x' seems to completely ignore the co_yield an never calls
  // promise.yield_value(). The coroutine seems to be resuming the 'co_await'
  // after the 'co_yield' rather than before the 'co_yield'. This bug is present
  // in VS 2017.7 and VS 2017.8.
