...
public:
  using coroutine_handle_t =
      cppcoro::coroutine_handle<when_all_task_promise<RESULT>>;
...
public:
  using coroutine_handle_t =
      cppcoro::coroutine_handle<when_all_task_promise<void>>;
...
template <typename AWAITABLE,
          typename RESULT =
              typename cppcoro::awaitable_traits<AWAITABLE &>::await_result_t,
          std::enable_if_t<!std::is_void_v<RESULT>, int> = 0>
when_all_task<RESULT>
make_when_all_task(std::reference_wrapper<AWAITABLE> awaitable) {
#if CPPCORO_COMPILER_MSVC && CPPCORO_COMPILER_MSVC < 19'20'00000
  // HACK: Workaround another bug in MSVC where the expression 'co_yield
  // co_await x' seems to completely ignore the co_yield and never calls
  // promise.yield_value(). The coroutine seems to be resuming the 'co_await'
  // after the 'co_yield' rather than before the 'co_yield'. This bug is present
  // in VS 2017.7 and VS 2017.8.
  auto &promise = co_await when_all_task_promise<RESULT>::get_promise;
