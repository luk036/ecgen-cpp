...
namespace cppcoro {
    namespace detail {
        ...
        template <typename OPERATION> class win32_overlapped_operation
        ...
        template <typename OPERATION> class win32_overlapped_operation_cancellable
            : protected win32_overlapped_operation_base {
          ...
          public:
            ...
            bool await_suspend(cppcoro::coroutine_handle<> awaitingCoroutine) {
                ...
                if (canBeCancelled) {
                    m_cancellationCallback.emplace(std::move(m_cancellationToken),
          ...
          private:
            enum class state { not_started, started, cancellation_requested, completed };
            ...
            void on_cancellation_requested() noexcept {
                ...
                if (oldState == state::not_started) {
                    ...
                    const bool transferredCancelResponsibility = m_state.compare_exchange_strong(
                        oldState, state::cancellation_requested, std::memory_order_release,
                        std::memory_order_acquire);
