...
class win32_overlapped_operation_base : protected detail::win32::io_state {
  public:
    win32_overlapped_operation_base(
        ...
        : detail::win32::io_state(callback), m_errorCode(0),
          m_numberOfBytesTransferred(0) {}

    win32_overlapped_operation_base(
        void *pointer,
        detail::win32::io_state::callback_type *callback) noexcept
        : detail::win32::io_state(pointer, callback), m_errorCode(0),
          m_numberOfBytesTransferred(0) {}

    win32_overlapped_operation_base(
        std::uint64_t offset,
        detail::win32::io_state::callback_type *callback) noexcept
        : detail::win32::io_state(offset, callback), m_errorCode(0),
          m_numberOfBytesTransferred(0) {}
...
class win32_overlapped_operation_cancellable
  ...
  protected:
    win32_overlapped_operation_cancellable(cancellation_token &&ct) noexcept
        : win32_overlapped_operation_base(
          ...
          m_state(ct.is_cancellation_requested() ? state::completed
                                                 : state::not_started),
          m_cancellationToken(std::move(ct)) {
        m_errorCode = error_operation_aborted;
    }

    win32_overlapped_operation_cancellable(void *pointer,
  ...
  public:
    ...
    CPPCORO_NOINLINE
    bool await_suspend(cppcoro::coroutine_handle<> awaitingCoroutine) {
        static_assert(std::is_base_of_v<win32_overlapped_operation_cancellable,
                                        OPERATION>);
        ...
        const bool willCompleteAsynchronously =
        ...
        if (canBeCancelled) {
            ...
            // another thread, transitioning directly from not_started ->
            // complete. Or it may have had the cancellation callback execute
            // and transition from not_started -> cancellation_requested. We use
            // a compare-exchange to determine a winner between these potential
            // racing cases.
            ...
            if (!m_state.compare_exchange_strong(oldState, state::started,
                ...
                if (oldState == state::cancellation_requested) {
                    ...
                    if (!m_state.compare_exchange_strong(
                            oldState, state::started, std::memory_order_release,
                            std::memory_order_acquire)) {
    ...
    decltype(auto) await_resume() {
        ...
        // has completed rather than waiting until the operation object
        // destructs. eg. If the operation is passed to when_all() then the
        // operation object may not be destructed until all of the operations
        // complete.
  ...
  private:
        ...
        if (oldState == state::not_started) {
            ...
            const bool transferredCancelResponsibility =
                m_state.compare_exchange_strong(
                    oldState, state::cancellation_requested,
    ...
    static void on_operation_completed(
        ...
        [[maybe_unused]] detail::win32::ulongptr_t completionKey) noexcept {
        auto *operation =
            static_cast<win32_overlapped_operation_cancellable *>(ioState);
        ...
        if (state == state::started) {
            operation->m_state.store(state::completed,
                                     std::memory_order_relaxed);
            operation->m_awaitingCoroutine.resume();
        } else {
            // We are racing with await_suspend() call suspending.
            // Try to mark it as completed using an atomic exchange and look
            // at the previous value to determine whether the coroutine
            // suspended first (in which case we resume it now) or we marked it
            // as completed first (in which case await_suspend() will return
            // false and immediately resume the coroutine).
            state = operation->m_state.exchange(state::completed,
                                                std::memory_order_acq_rel);
            if (state == state::started) {
                // The await_suspend() method returned (or will return) 'true'
                // and so we need to resume the coroutine.
                operation->m_awaitingCoroutine.resume();
            }
        }
    }
