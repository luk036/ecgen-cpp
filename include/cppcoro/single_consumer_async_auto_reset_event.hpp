  ...
  public:
    ...
    void set() noexcept {
        ...
        if (oldValue != nullptr && oldValue != this) {
            ...
            // concurrently with this call and execute the above
            // m_state.exchange(this) operation with 'release' semantics. This
            // needs to be an exchange() instead of a store() so that it can
    ...
    auto operator co_await() const noexcept {
        class awaiter {
          public:
            awaiter(
                const single_consumer_async_auto_reset_event &event) noexcept
                : m_event(event) {}
            ...
            bool await_suspend(
                ...
                if (!m_event.m_state.compare_exchange_strong(
                        oldValue, &m_awaitingCoroutine,
                        std::memory_order_release, std::memory_order_relaxed)) {
                    // This will only fail if the event was already 'set'
                    // In which case we can just reset back to 'not set'
                    // Need to use exchange() rather than store() here so we can
                    // make this operation an 'acquire' operation so that we get
                    // visibility of all writes prior to all preceding calls to
                    // .set().
                    assert(oldValue == &m_event);
                    (void)m_event.m_state.exchange(nullptr,
