...
public:
  ...
  void set() noexcept {
    ...
    if (oldValue != nullptr && oldValue != this) {
      ...
      // m_state.exchange(this) operation with 'release' semantics. This needs
      // to be an exchange() instead of a store() so that it can have 'acquire'
      // semantics.
    ...
    public:
      ...
      await_suspend(cppcoro::coroutine_handle<> awaitingCoroutine) noexcept {
        ...
        if (!m_event.m_state.compare_exchange_strong(
                oldValue, &m_awaitingCoroutine, std::memory_order_release,
