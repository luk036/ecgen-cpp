...
public:
  ...
  single_producer_sequencer(
      ...
      : m_consumerBarrier(consumerBarrier), m_bufferSize(bufferSize),
        m_nextToClaim(initialSequence + 1), m_producerBarrier(initialSequence) {
  }
  ...
  /// elements to enqueue. This will claim as many slots as are available up to
  /// the specified count but may claim as few as one slot if only one slot is
  /// available.
  ///
  /// \param count
  ...
  /// have written element values to all of the claimed slots you must publish()
  /// the sequence range in order to make the elements available to consumers.
  template <typename SCHEDULER>
  ...
  ///
  /// The result of the 'co_await barrier.wait_until_published(seq)' expression
  /// will be the last-published sequence number, which is guaranteed to be at
  /// least 'seq' but may be some subsequent sequence number if additional items
  /// were published while waiting for the the requested sequence number to be
  /// published.
  template <typename SCHEDULER>
...
public:
  single_producer_sequencer_claim_one_operation(
      ...
      : m_consumerWaitOperation(sequencer.m_consumerBarrier,
