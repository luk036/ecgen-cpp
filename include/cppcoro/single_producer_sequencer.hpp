...
class single_producer_sequencer {
  public:
    ...
    single_producer_sequencer(
        ...
        : m_consumerBarrier(consumerBarrier), m_bufferSize(bufferSize),
    ...
    /// The caller must publish() the claimed sequence number once they have
    /// written to the ring-buffer.
    template <typename SCHEDULER>
    [[nodiscard]] single_producer_sequencer_claim_one_operation<
        SEQUENCE, TRAITS, SCHEDULER>
    claim_one(SCHEDULER &scheduler) noexcept;

    /// Claim one or more contiguous slots in the ring-buffer.
    ///
    /// Use this method over many calls to claim_one() when you have multiple
    /// elements to enqueue. This will claim as many slots as are available up
    /// to the specified count but may claim as few as one slot if only one slot
    /// is available.
    ///
    /// \param count
    ...
    ///
    /// \return
    /// Returns an awaitable object that when awaited returns a sequence_range
    /// that contains the range of sequence numbers that were claimed. Once you
    /// have written element values to all of the claimed slots you must
    /// publish() the sequence range in order to make the elements available to
    /// consumers.
    template <typename SCHEDULER>
    [[nodiscard]] single_producer_sequencer_claim_operation<SEQUENCE, TRAITS,
                                                            SCHEDULER>
    claim_up_to(std::size_t count, SCHEDULER &scheduler) noexcept;

    /// Publish the specified sequence number.
    ///
    /// This also implies that all prior sequence numbers have already been
    /// published.
    ...
    ///
    /// \return
    /// Returns an Awaitable type that, when awaited, will suspend the awaiting
    /// coroutine until the specified sequence number has been published.
    ///
    /// The result of the 'co_await barrier.wait_until_published(seq)'
    /// expression will be the last-published sequence number, which is
    /// guaranteed to be at least 'seq' but may be some subsequent sequence
    /// number if additional items were published while waiting for the the
...
class single_producer_sequencer_claim_one_operation {
  public:
    single_producer_sequencer_claim_one_operation(
        ...
        : m_consumerWaitOperation(
              ...
              static_cast<SEQUENCE>(sequencer.m_nextToClaim -
                                    sequencer.m_bufferSize),
              scheduler),
  ...
  private:
    sequence_barrier_wait_operation<SEQUENCE, TRAITS, SCHEDULER>
        m_consumerWaitOperation;
    ...
    explicit single_producer_sequencer_claim_operation(
        ...
        : m_consumerWaitOperation(
              ...
              static_cast<SEQUENCE>(sequencer.m_nextToClaim -
                                    sequencer.m_bufferSize),
              scheduler),
          m_sequencer(sequencer), m_count(count) {}
...
template <typename SCHEDULER>
[[nodiscard]] single_producer_sequencer_claim_one_operation<SEQUENCE, TRAITS,
                                                            SCHEDULER>
single_producer_sequencer<SEQUENCE, TRAITS>::claim_one(
    SCHEDULER &scheduler) noexcept {
    return single_producer_sequencer_claim_one_operation<SEQUENCE, TRAITS,
                                                         SCHEDULER>{*this,
                                                                    scheduler};
}
...
single_producer_sequencer<SEQUENCE, TRAITS>::claim_up_to(
    std::size_t count, SCHEDULER &scheduler) noexcept {
    return single_producer_sequencer_claim_operation<SEQUENCE, TRAITS,
