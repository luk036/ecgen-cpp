...
class async_generator_yield_operation final {
  public:
    async_generator_yield_operation(
        cppcoro::coroutine_handle<> consumer) noexcept
        : m_consumer(consumer) {}

    bool await_ready() const noexcept { return false; }

    cppcoro::coroutine_handle<> await_suspend(
        [[maybe_unused]] cppcoro::coroutine_handle<> producer) noexcept {
        return m_consumer;
    }
...
template <typename T>
class async_generator_promise<T &&> final
...
template <typename T> class [[nodiscard]] async_generator {
  public:
    ...
    explicit async_generator(promise_type &promise) noexcept
        : m_coroutine(
              cppcoro::coroutine_handle<promise_type>::from_promise(promise)) {}
...
class async_generator_promise_base {
  public:
    ...
    /// associated producer coroutine is now available to be destroyed. In which
    /// case the caller is expected to call destroy() on the coroutine_handle.
    /// Returns false if the producer coroutine was not at a suitable
    /// suspend-point. The coroutine will be destroyed when it next reaches a
    ...
    bool request_cancellation() noexcept {
        ...
        // Not valid to destroy async_generator<T> object if consumer coroutine
        // still suspended in a co_await for next item.
        assert(previousState != state::value_not_ready_consumer_suspended);

        // A coroutine should only ever be cancelled once, from the destructor
        // of the owning async_generator<T> object.
...
async_generator_promise_base::internal_yield_value() noexcept {
    ...
    if (currentState == state::value_not_ready_consumer_suspended) {
        ...
        m_state.store(state::value_ready_producer_active,
                      std::memory_order_relaxed);

        // Resume the consumer.
        // It might ask for another value before returning, in which case it'll
        // transition to value_not_ready_consumer_suspended and we can return
        // from yield_value without suspending, otherwise we should try to
        // suspend the producer in which case the consumer will wake us up again
...
inline bool async_generator_yield_operation::await_suspend(
    ...
    if (currentState == state::value_not_ready_consumer_active) {
        bool producerSuspended = m_promise.m_state.compare_exchange_strong(
            currentState, state::value_ready_producer_suspended,
        ...
        if (currentState == state::value_not_ready_consumer_suspended) {
            ...
            m_promise.m_state.store(state::value_ready_producer_active,
                                    std::memory_order_relaxed);

            m_promise.m_consumerCoroutine.resume();

            // The consumer might have asked for another value before returning,
            // in which case it'll transition to
            // value_not_ready_consumer_suspended and we can return without
            // suspending, otherwise we should try to suspend the producer, in
            // which case the consumer will wake us up again when it wants the
            // next value.
            //
            // Need to use acquire semantics here since it's possible that the
            // consumer might have asked for the next value on a different
            // thread which executed concurrently with the call to
            // m_consumerCoro.resume() above.
    ...
    if (currentState == state::value_ready_producer_active) {
        ...
        // destructed, transitioning the state to cancelled, or requested the
        // next item, transitioning the state to
        // value_not_ready_consumer_suspended.
        const bool suspendedProducer =
            m_promise.m_state.compare_exchange_strong(
                currentState, state::value_ready_producer_suspended,
                std::memory_order_release, std::memory_order_acquire);
  ...
  protected:
    ...
    async_generator_advance_operation(
        ...
        if (initialState == state::value_ready_producer_suspended) {
            ...
            promise.m_state.store(state::value_not_ready_consumer_active,
                                  std::memory_order_relaxed);

            producerCoroutine.resume();

            // Need to use acquire memory order here since it's possible that
            // the coroutine may have transferred execution to another thread
            // and completed on that other thread before the call to resume()
            // returns.
    ...
    bool await_suspend(cppcoro::coroutine_handle<> consumerCoroutine) noexcept {
        ...
        if (currentState == state::value_ready_producer_active) {
            ...
            if (m_promise->m_state.compare_exchange_strong(
                    currentState, state::value_not_ready_consumer_suspended,
                    std::memory_order_release, std::memory_order_acquire)) {
                return true;
            }
            ...
            m_promise->m_state.store(state::value_not_ready_consumer_active,
                                     std::memory_order_relaxed);
        ...
        // producer making the next value available and suspending (transition
        // to value_ready_producer_suspended) so we use compare_exchange to
        // decide who wins the race. If compare_exchange succeeds then consumer
        // suspended (and we return true). If it fails then producer yielded
        // next value and suspended and we can return synchronously without
        // suspended (ie. return false).
