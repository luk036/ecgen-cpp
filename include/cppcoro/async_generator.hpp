...
public:
  ...
  /// producer coroutine is now available to be destroyed. In which case the
  /// caller is expected to call destroy() on the coroutine_handle. Returns
  /// false if the producer coroutine was not at a suitable suspend-point. The
  /// coroutine will be destroyed when it next reaches a co_yield or co_return
  /// statement.
  bool request_cancellation() noexcept {
    ...
    // A coroutine should only ever be cancelled once, from the destructor of
    // the owning async_generator<T> object.
...
inline bool async_generator_yield_operation::await_suspend(
  ...
  if (currentState == state::value_not_ready_consumer_active) {
    ...
    if (currentState == state::value_not_ready_consumer_suspended) {
      ...
      // The consumer might have asked for another value before returning, in
      // which case it'll transition to value_not_ready_consumer_suspended and
      // we can return without suspending, otherwise we should try to suspend
      // the producer, in which case the consumer will wake us up again when it
      // wants the next value.
      //
      // Need to use acquire semantics here since it's possible that the
      // consumer might have asked for the next value on a different thread
      // which executed concurrently with the call to m_consumerCoro.resume()
      // above.
  ...
  if (currentState == state::value_ready_producer_active) {
    ...
    // transitioning the state to cancelled, or requested the next item,
    // transitioning the state to value_not_ready_consumer_suspended.
...
public:
  ...
  bool await_suspend(cppcoro::coroutine_handle<> consumerCoroutine) noexcept {
    ...
    // value_not_ready_consumer_suspended. This could be racing with producer
    // making the next value available and suspending (transition to
    // value_ready_producer_suspended) so we use compare_exchange to decide who
    // wins the race.
    // If compare_exchange succeeds then consumer suspended (and we return
    // true). If it fails then producer yielded next value and suspended and we
    // can return synchronously without suspended (ie. return false).
