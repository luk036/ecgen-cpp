...
/// A sequence barrier is a synchronisation primitive that allows a
/// single-producer and multiple-consumers to coordinate with respect to a
/// monotonically increasing sequence number.
///
/// A single producer advances the sequence number by publishing new sequence
/// numbers in a monotonically increasing order. One or more consumers can query
/// the last-published sequence number and can wait until a particular sequence
/// number has been published.
///
/// A sequence barrier can be used to represent a cursor into a thread-safe
...
class sequence_barrier {
    static_assert(std::is_integral_v<SEQUENCE>,
  ...
  public:
    ...
    sequence_barrier(
        ...
        : m_lastPublished(initialSequence), m_awaiters(nullptr) {}

    ~sequence_barrier() {
        // Shouldn't be destructing a sequence barrier if there are still
        // waiters.
        assert(m_awaiters.load(std::memory_order_relaxed) == nullptr);
    }

    /// Query the sequence number that was most recently published by the
    /// producer.
    ///
    /// You can assume that all sequence numbers prior to the returned sequence
    /// number have also been published. This means you can safely access all
    /// elements with sequence numbers up to and including the returned sequence
    /// number without any further synchronisation.
    ...
    ///
    /// If the specified sequence number is not yet published then the awaiting
    /// coroutine will be suspended and later resumed inside the call to
    /// publish() that publishes the specified sequence number.
    ///
    /// \param targetSequence
    /// The sequence number to wait for.
    ///
    /// \return
    /// An awaitable that when co_await'ed will suspend the awaiting coroutine
    /// until the specified target sequence number has been published. The
    /// result of the co_await expression will be the last-known published
    /// sequence number. This is guaranteed not to precede \p targetSequence but
    /// may be a sequence number after \p targetSequence, which indicates that
    ...
    wait_until_published(SEQUENCE targetSequence,
                         SCHEDULER &scheduler) const noexcept;

    /// Publish the specified sequence number to consumers.
    ///
    /// This publishes all sequence numbers up to and including the specified
    /// sequence number. This will resume any coroutine that was suspended
    /// waiting for a sequence number that was published by this operation.
    ///
    /// \param sequence
    /// The sequence number to publish. This number must not precede the current
    /// last_published() value. ie. the published sequence numbers must be
...
class sequence_barrier_wait_operation_base {
  public:
    explicit sequence_barrier_wait_operation_base(
        ...
        : m_barrier(barrier), m_targetSequence(targetSequence),
    ...
    sequence_barrier_wait_operation_base(
        ...
        : m_barrier(other.m_barrier), m_targetSequence(other.m_targetSequence),
  ...
  protected:
    ...
    void resume() noexcept {
        // This synchronises with the exchange(true, std::memory_order_acquire)
        // in await_suspend().
...
class sequence_barrier_wait_operation
  ...
  public:
    sequence_barrier_wait_operation(
        ...
        : sequence_barrier_wait_operation_base<SEQUENCE, TRAITS>(
              barrier, targetSequence),
    ...
    decltype(auto)
    await_resume() noexcept(noexcept(m_scheduleAwaiter->await_resume())) {
        if (m_isScheduleAwaiterCreated) {
            m_scheduleAwaiter->await_resume();
        }

        return sequence_barrier_wait_operation_base<SEQUENCE,
                                                    TRAITS>::await_resume();
    }
...
void sequence_barrier<SEQUENCE, TRAITS>::publish(SEQUENCE sequence) noexcept {
    ...
    // Note we may be racing with add_awaiter() which could also acquire the
    // list of waiters so we need to check again whether we won the race and
    // acquired the list.
...
template <typename SEQUENCE, typename TRAITS>
void sequence_barrier<SEQUENCE, TRAITS>::add_awaiter(
    awaiter_t *awaiter) const noexcept {
    ...
    do {
        ...
        // This needs to be seq_cst memory order to ensure that in the case that
        // the producer publishes a new sequence number concurrently with this
        // call that we either see their write to m_lastPublished after
        // enqueueing our awaiter, or they see our write to m_awaiters after
        // their write to m_lastPublished.
        ...
        // At least one of the awaiters we just enqueued is now satisfied by a
        // concurrently published sequence number. The producer thread may not
        ...
        while (awaiters != nullptr) {
            const auto diff = TRAITS::difference(awaiters->m_targetSequence,
                                                 lastKnownPublished);
            if (diff > 0) {
                *awaitersToRequeueTail = awaiters;
                awaitersToRequeueTail = &awaiters->m_next;
        ...
        *awaitersToRequeueTail = nullptr;
        ...
        // Calculate the earliest target sequence required by any of the
        // awaiters to requeue.
