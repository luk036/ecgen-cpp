...
public:
  ...
  /// coroutine will be suspended and later resumed inside the call to publish()
  /// that publishes the specified sequence number.
  ///
  ...
  /// until the specified target sequence number has been published. The result
  /// of the co_await expression will be the last-known published sequence
  /// number. This is guaranteed not to precede \p targetSequence but may be a
  /// sequence number after \p targetSequence, which indicates that more
  /// elements have been published than you were waiting for.
  ...
  /// sequence number. This will resume any coroutine that was suspended waiting
  /// for a sequence number that was published by this operation.
  ///
  /// \param sequence
...
public:
  explicit sequence_barrier_wait_operation_base(
      ...
      : m_barrier(barrier), m_targetSequence(targetSequence),
        m_lastKnownPublished(barrier.last_published()), m_readyToResume(false) {
  }
...
public:
  sequence_barrier_wait_operation(
      ...
      : sequence_barrier_wait_operation_base<SEQUENCE, TRAITS>(barrier,
                                                               targetSequence),
...
void sequence_barrier<SEQUENCE, TRAITS>::publish(SEQUENCE sequence) noexcept {
  ...
  // Note we may be racing with add_awaiter() which could also acquire the list
  // of waiters so we need to check again whether we won the race and acquired
  // the list.
  ...
  // Check the list of awaiters for ones that are now satisfied by the sequence
  // number we just published. Awaiters are added to either the
...
void sequence_barrier<SEQUENCE, TRAITS>::add_awaiter(
  ...
  do {
    ...
    // This needs to be seq_cst memory order to ensure that in the case that the
    // producer publishes a new sequence number concurrently with this call that
    // we either see their write to m_lastPublished after enqueueing our
    // awaiter, or they see our write to m_awaiters after their write to
    // m_lastPublished.
    ...
    // concurrently published sequence number. The producer thread may not have
    // seen our write to m_awaiters so we need to try to re-acquire the list of
    // awaiters to ensure that the waiters that are now satisfied are woken up.
    ...
    *awaitersToRequeueTail = nullptr;
    ...
    // Calculate the earliest target sequence required by any of the awaiters to
    // requeue.
