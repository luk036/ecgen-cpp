...
class multi_producer_sequencer {
  public:
    ...
    /// coroutine until the specified 'targetSequence' number and all prior
    /// sequence numbers have been published.
    template <typename SCHEDULER>
    multi_producer_sequencer_wait_operation<SEQUENCE, TRAITS, SCHEDULER>
    wait_until_published(SEQUENCE targetSequence, SEQUENCE lastKnownPublished,
                         SCHEDULER &scheduler) const noexcept;

    /// Query if there are currently any slots available for claiming.
    ///
    /// Note that this return-value is only approximate if you have multiple
    /// producers since immediately after returning true another thread may have
    /// claimed the last available slot.
    bool any_available() const noexcept;

    /// Claim a single slot in the buffer and wait until that slot becomes
    /// available.
    ///
    /// Returns an Awaitable type that yields the sequence number of the slot
    /// that was claimed.
    ///
    /// Once the producer has claimed a slot then they are free to write to that
    /// slot within the ring buffer. Once the value has been initialised the
    /// item must be published by calling the .publish() method, passing the
    /// sequence number.
    template <typename SCHEDULER>
    multi_producer_sequencer_claim_one_operation<SEQUENCE, TRAITS, SCHEDULER>
    claim_one(SCHEDULER &scheduler) noexcept;

    /// Claim a contiguous range of sequence numbers corresponding to slots
    /// within a ring-buffer.
    ///
    /// This will claim at most the specified count of sequence numbers but may
    /// claim fewer if there are only fewer entries available in the buffer. But
    /// will claim at least one sequence number.
    ///
    /// Returns an awaitable that will yield a sequence_range object containing
    /// the sequence numbers that were claimed.
    ///
    /// The caller is responsible for ensuring that they publish every element
    /// of the returned sequence range by calling .publish().
    template <typename SCHEDULER>
    ...
    /// Publish the element with the specified sequence number, making it
    /// available to consumers.
    ///
    /// Note that different sequence numbers may be published by different
    /// producer threads out of order. A sequence number will not become
    /// available to consumers until all preceding sequence numbers have also
    /// been published.
    ///
    /// \param sequence
    /// The sequence number of the elemnt to publish
    /// This sequence number must have been previously acquired via a call to
    /// 'claim_one()' or 'claim_up_to()'.
    void publish(SEQUENCE sequence) noexcept;

    /// Publish a contiguous range of sequence numbers, making each of them
    /// available to consumers.
    ///
    /// This is equivalent to calling publish(seq) for each sequence number,
    /// seq, in the specified range, but is more efficient since it only checks
    /// to see if there are coroutines that need to be woken up once.
  ...
  private:
    ...
    void add_awaiter(
        multi_producer_sequencer_wait_operation_base<SEQUENCE, TRAITS> *awaiter)
        const noexcept;

#if CPPCORO_COMPILER_MSVC
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4324) // C4324: structure was padded due to alignment specifier
#endif
  ...
  public:
    multi_producer_sequencer_claim_operation(
        ...
        : m_sequencer(sequencer),
          m_count(count < sequencer.buffer_size() ? count
                                                  : sequencer.buffer_size()),
          m_scheduler(scheduler) {}

    multi_producer_sequencer_claim_awaiter<SEQUENCE, TRAITS, SCHEDULER>
    operator co_await() noexcept {
        // We wait until the awaitable is actually co_await'ed before we claim
        // the range of elements. If we claimed them earlier, then it may be
        // possible for the caller to fail to co_await the result eg. due to an
        // exception, which would leave the sequence numbers unable to be
        // published and would eventually deadlock consumers that waited on
        // them.
        //
        // TODO: We could try and acquire only as many as are available if fewer
        // than m_count elements are available. This would complicate the logic
        // here somewhat as we'd need to use a compare-exchange instead.
        const SEQUENCE first = m_sequencer.m_nextToClaim.fetch_add(
            m_count, std::memory_order_relaxed);
        return multi_producer_sequencer_claim_awaiter<SEQUENCE, TRAITS,
                                                      SCHEDULER>{
            m_sequencer.m_consumerBarrier, m_sequencer.buffer_size(),
...
class multi_producer_sequencer_claim_one_awaiter {
  public:
    multi_producer_sequencer_claim_one_awaiter(
        const sequence_barrier<SEQUENCE, TRAITS> &consumerBarrier,
        std::size_t bufferSize, SEQUENCE claimedSequence,
        SCHEDULER &scheduler) noexcept
...
class multi_producer_sequencer_claim_one_operation {
  public:
    multi_producer_sequencer_claim_one_operation(
        multi_producer_sequencer<SEQUENCE, TRAITS> &sequencer,
        SCHEDULER &scheduler) noexcept
        : m_sequencer(sequencer), m_scheduler(scheduler) {}
...
class multi_producer_sequencer_wait_operation_base {
  public:
    multi_producer_sequencer_wait_operation_base(
        ...
        SEQUENCE targetSequence, SEQUENCE lastKnownPublished) noexcept
        : m_sequencer(sequencer), m_targetSequence(targetSequence),
          m_lastKnownPublished(lastKnownPublished), m_readyToResume(false) {}
    ...
    bool await_suspend(cppcoro::coroutine_handle<> awaitingCoroutine) noexcept {
        ...
        // If it was already marked as ready-to-resume within the call to
        // add_awaiter() or on another thread then this exchange() will return
        // true. In this case we want to resume immediately and continue
...
class multi_producer_sequencer_wait_operation
  ...
  public:
    multi_producer_sequencer_wait_operation(
        const multi_producer_sequencer<SEQUENCE, TRAITS> &sequencer,
        SEQUENCE targetSequence, SEQUENCE lastKnownPublished,
        SCHEDULER &scheduler) noexcept
        : multi_producer_sequencer_wait_operation_base<SEQUENCE, TRAITS>(
    ...
    SEQUENCE
    await_resume() noexcept(noexcept(m_scheduleOperation->await_resume())) {
        if (m_isScheduleOperationCreated) {
            m_scheduleOperation->await_resume();
        }

        return multi_producer_sequencer_wait_operation_base<
            SEQUENCE, TRAITS>::await_resume();
    }
  ...
  private:
        ...
        try {
            ...
            if (!m_scheduleAwaiter->await_ready()) {
                using await_suspend_result_t =
                    decltype(m_scheduleAwaiter->await_suspend(
                        this->m_awaitingCoroutine));
                if constexpr (std::is_void_v<await_suspend_result_t>) {
                    m_scheduleAwaiter->await_suspend(this->m_awaitingCoroutine);
                    return;
                } else if constexpr (std::is_same_v<await_suspend_result_t,
                                                    bool>) {
                    if (m_scheduleAwaiter->await_suspend(
                            this->m_awaitingCoroutine)) {
                        return;
                    }
                } else {
            ...
            // Or should we require that 'co_await scheduler.schedule()' is
            // noexcept?
        }
    ...
    detail::manual_lifetime<
        typename awaitable_traits<schedule_operation>::awaiter_t>
        m_scheduleAwaiter;
...
multi_producer_sequencer<SEQUENCE, TRAITS>::multi_producer_sequencer(
    const sequence_barrier<SEQUENCE, TRAITS> &consumerBarrier,
    std::size_t bufferSize, SEQUENCE initialSequence)
    : m_consumerBarrier(consumerBarrier), m_sequenceMask(bufferSize - 1),
      m_published(std::make_unique<std::atomic<SEQUENCE>[]>(bufferSize)),
      m_nextToClaim(initialSequence + 1), m_awaiters(nullptr) {
    ...
    do {
       ...
       // !__cpp_lib_atomic_value_initialization vvv
...
template <typename SEQUENCE, typename TRAITS>
bool multi_producer_sequencer<SEQUENCE, TRAITS>::any_available()
    const noexcept {
    return TRAITS::precedes(m_nextToClaim.load(std::memory_order_relaxed),
...
template <typename SCHEDULER>
multi_producer_sequencer_claim_one_operation<SEQUENCE, TRAITS, SCHEDULER>
multi_producer_sequencer<SEQUENCE, TRAITS>::claim_one(
    SCHEDULER &scheduler) noexcept {
    return multi_producer_sequencer_claim_one_operation<SEQUENCE, TRAITS,
                                                        SCHEDULER>{*this,
                                                                   scheduler};
}
...
multi_producer_sequencer<SEQUENCE, TRAITS>::claim_up_to(
    std::size_t count, SCHEDULER &scheduler) noexcept {
    return multi_producer_sequencer_claim_operation<SEQUENCE, TRAITS,
                                                    SCHEDULER>{*this, count,
                                                               scheduler};
}
...
void multi_producer_sequencer<SEQUENCE, TRAITS>::publish(
    ...
    m_published[sequence & m_sequenceMask].store(sequence,
                                                 std::memory_order_seq_cst);

    // Resume any waiters that might have been satisfied by this publish
    // operation.
...
void multi_producer_sequencer<SEQUENCE, TRAITS>::publish(
    ...
    // No consumer should be reading those subsequent sequence numbers until
    // they've seen that the first sequence number in the range is published.
    for (SEQUENCE seq : range.skip(1)) {
        m_published[seq & m_sequenceMask].store(seq, std::memory_order_relaxed);
    }

    // Now publish the first sequence number with seq_cst semantics.
    m_published[range.front() & m_sequenceMask].store(
        range.front(), std::memory_order_seq_cst);
...
template <typename SEQUENCE, typename TRAITS>
void multi_producer_sequencer<SEQUENCE,
                              TRAITS>::resume_ready_awaiters() noexcept {
    using awaiter_t =
        multi_producer_sequencer_wait_operation_base<SEQUENCE, TRAITS>;
    ...
    if (awaiters == nullptr) {
        ...
        // Some other thread is now responsible for resuming them. Our job is
        // done.
        return;
    }
    ...
    do {
        ...
        lastKnownPublished =
            last_published_after(awaiters->m_lastKnownPublished);

        // First scan the list of awaiters and split them into 'requeue' and
        // 'resume' lists.
        auto minDiff = std::numeric_limits<diff_t>::max();
        do {
            auto diff = TRAITS::difference(awaiters->m_targetSequence,
                                           lastKnownPublished);
            if (diff > 0) {
                // Not ready yet.
        ...
        *awaitersToRequeueTail = nullptr;
        ...
        if (awaitersToRequeue != nullptr) {
            ...
            while (!m_awaiters.compare_exchange_weak(
                oldHead, awaitersToRequeue, std::memory_order_seq_cst,
            ...
            const SEQUENCE earliestTargetSequence =
                lastKnownPublished + minDiff;

            // Now we need to check again to see if any of the waiters we just
            // enqueued is now satisfied by a concurrent call to publish().
            //
            // We need to be a bit more careful here since we are no longer
            // holding any awaiters and so producers/consumers may advance the
            // sequence number arbitrarily far. If the sequence number advances
            // more than buffer_size() ahead of the earliestTargetSequence then
            // the m_published[] array may have sequence numbers that have
            // advanced beyond earliestTargetSequence, potentially even wrapping
            // sequence numbers around to then be preceding where they were
            // before. If this happens then we don't need to worry about
            // resuming any awaiters that were waiting for
            // 'earliestTargetSequence' since some other thread has already
            // resumed them. So the only case we need to worry about here is
            // when all m_published entries for sequence numbers in range
            // [lastKnownPublished + 1, earliestTargetSequence] have published
            // sequence numbers that match the range.
            const auto sequenceMask = m_sequenceMask;
            SEQUENCE seq = lastKnownPublished + 1;
            while (m_published[seq & sequenceMask].load(
                       std::memory_order_seq_cst) == seq) {
                lastKnownPublished = seq;
                if (seq == earliestTargetSequence) {
                    // At least one of the awaiters we just published is now
                    // satisfied. Reacquire the list of awaiters and continue
                    // around the outer loop.
                    awaiters =
                        m_awaiters.exchange(nullptr, std::memory_order_acquire);
                    break;
                }
                ++seq;
            }
...
void multi_producer_sequencer<SEQUENCE, TRAITS>::add_awaiter(
    ...
    do {
        // Enqueue the awaiters.
        {
            ...
            do {
                *awaitersToEnqueueTail = oldHead;
                ...
                oldHead, awaitersToEnqueue, std::memory_order_seq_cst,
                std::memory_order_relaxed));
        }
        ...
        // while we were enqueuing the awaiters. Need to use seq_cst memory
        // order here to ensure that if there are concurrent calls to publish()
        // that would wake up any of the awaiters we just enqueued that either
        // we will see their write to m_published slots or they will see our
        // write to m_awaiters.
        //
        // Note also, that we are assuming that the last-known published
        // sequence is not going to advance more than buffer_size() ahead of
        // targetSequence since there is at least one consumer that won't be
        // resumed and so thus can't publish the sequence number it's waiting
        // for to its sequence_barrier and so producers won't be able to claim
        // its slot in the buffer.
        //
        // TODO: Check whether we can weaken the memory order here to just use
        // 'seq_cst' on the first .load() and then use 'acquire' on subsequent
        // .load().
        ...
        if (!TRAITS::precedes(lastKnownPublished, targetSequence)) {
            ...
            // To ensure it is woken up we need to reacquire the list of
            // awaiters and resume
            awaiter_t *awaiters =
                m_awaiters.exchange(nullptr, std::memory_order_acquire);
            ...
            while (awaiters != nullptr) {
                diff_t diff =
                    TRAITS::difference(targetSequence, lastKnownPublished);
                if (diff > 0) {
                    // Not yet ready.
                    minDiff = diff < minDiff ? diff : minDiff;
            ...
            // around the loop.
            targetSequence =
                static_cast<SEQUENCE>(lastKnownPublished + minDiff);
        }
    ...
    *awaitersToResumeTail = nullptr;
    ...
    while (awaitersToResume != nullptr) {
        // Read m_next before calling .resume() as resuming could destroy the
        // awaiter.
        awaiter_t *next = awaitersToResume->m_next;
        awaitersToResume->resume(lastKnownPublished);
        awaitersToResume = next;
