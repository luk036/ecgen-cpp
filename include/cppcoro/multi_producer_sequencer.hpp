...
public:
  ...
  ///
  /// The caller is responsible for ensuring that they publish every element of
  /// the returned sequence range by calling .publish().
  template <typename SCHEDULER>
  ...
  /// producer threads out of order. A sequence number will not become available
  /// to consumers until all preceding sequence numbers have also been
  /// published.
  ///
  /// \param sequence
  ...
  /// available to consumers.
  ///
  /// This is equivalent to calling publish(seq) for each sequence number, seq,
  /// in the specified range, but is more efficient since it only checks to see
  /// if there are coroutines that need to be woken up once.
...
public:
  ...
  operator co_await() noexcept {
    ...
    // range of elements. If we claimed them earlier, then it may be possible
    // for the caller to fail to co_await the result eg. due to an exception,
    // which would leave the sequence numbers unable to be published and would
    // eventually deadlock consumers that waited on them.
    //
    // TODO: We could try and acquire only as many as are available if fewer
    // than m_count elements are available. This would complicate the logic here
    // somewhat as we'd need to use a compare-exchange instead.
...
public:
  ...
  bool await_suspend(cppcoro::coroutine_handle<> awaitingCoroutine) noexcept {
    ...
    // add_awaiter() or on another thread then this exchange() will return true.
    // In this case we want to resume immediately and continue execution by
    // returning false.
...
public:
  ...
  await_resume() noexcept(noexcept(m_scheduleOperation->await_resume())) {
    ...
    return multi_producer_sequencer_wait_operation_base<SEQUENCE,
                                                        TRAITS>::await_resume();
  }
...
multi_producer_sequencer<SEQUENCE, TRAITS>::claim_up_to(
  ...
  return multi_producer_sequencer_claim_operation<SEQUENCE, TRAITS, SCHEDULER>{
...
void multi_producer_sequencer<SEQUENCE, TRAITS>::publish(
  ...
  m_published[range.front() & m_sequenceMask].store(range.front(),
                                                    std::memory_order_seq_cst);
  ...
  do {
    ...
    *awaitersToRequeueTail = nullptr;
    ...
    if (awaitersToRequeue != nullptr) {
      ...
      //
      // We need to be a bit more careful here since we are no longer holding
      // any awaiters and so producers/consumers may advance the sequence number
      // arbitrarily far. If the sequence number advances more than
      // buffer_size() ahead of the earliestTargetSequence then the
      // m_published[] array may have sequence numbers that have advanced beyond
      // earliestTargetSequence, potentially even wrapping sequence numbers
      // around to then be preceding where they were before. If this happens
      // then we don't need to worry about resuming any awaiters that were
      // waiting for 'earliestTargetSequence' since some other thread has
      // already resumed them. So the only case we need to worry about here is
      ...
      SEQUENCE seq = lastKnownPublished + 1;
      while (m_published[seq & sequenceMask].load(std::memory_order_seq_cst) ==
             seq) {
        lastKnownPublished = seq;
...
void multi_producer_sequencer<SEQUENCE, TRAITS>::add_awaiter(
  ...
  do {
    ...
    // wake up any of the awaiters we just enqueued that either we will see
    // their write to m_published slots or they will see our write to
    // m_awaiters.
    //
    // Note also, that we are assuming that the last-known published sequence is
    // not going to advance more than buffer_size() ahead of targetSequence
    // since there is at least one consumer that won't be resumed and so thus
    // can't publish the sequence number it's waiting for to its
    // sequence_barrier and so producers won't be able to claim its slot in the
    // buffer.
    //
    ...
    if (!TRAITS::precedes(lastKnownPublished, targetSequence)) {
      ...
      // To ensure it is woken up we need to reacquire the list of awaiters and
      // resume
      awaiter_t *awaiters =
