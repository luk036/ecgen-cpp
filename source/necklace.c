...
int IsCharm()
{
    ...
    for (i = 2; i < N - 1; i++)
    {
        ...
        if (PRIME[i])
        {
            ...
            for (j = 1; j <= N; j++)
            {
                if (a[j] < charm[offset + j - 1])
                    break;
                else if (a[j] > charm[offset + j - 1])
                    return FALSE;
            }
        }
    }
...
int CheckRevF()
{
    ...
    j = 1;
    while (
        B[j].v == B[nb - j + 1].v && B[j].s == B[nb - j + 1].s && j <= nb / 2)
        j++;
    if (j > nb / 2)
        return 0;
    if (B[j].s < B[nb - j + 1].s)
...
void GenA(int t, int p, int s)
{
    ...
    if (num[K] == N - t + 1)
    {
        if (NECK && (num[K] == run[t - p]) && (N % p == 0))
            Print();
        else if (LYN && (num[K] == run[t - p]) && (N == p))
            Print();
        else if (num[K] > run[t - p])
            Print();
    }
    else if (num[1] != N - t + 1)
    {
...
void GenC2(int s, int t, int p, int T, int P, int len, int num_sec)
{
    ...
    else if (s > (num_sec + 1) * P)
    {
        pos[len][t] = P;
        if (min == P)
            GenC2(s, t + 1, p, T, P, len, num_sec + 1);
        else
            GenC2(s, t + 1, t, T, P, len, num_sec + 1);
    }
    else if (s == N * 2)
    {
        if (T == N)
            ...
            GenRestC(head, avail[head].next, len + 1);
        else if (t % p == 0 && len < N)
            GenC2(head, 1, 1, T, N * 2 * p / t, len + 1, 0);
    }
    else
    {
        e = (s + len) % (N * 2);
        if (s % P >= min && avail[e].next != e)
        {
            ...
            pos[len][t] = s % P;
            if (s % P == min)
                GenC2(next, t + 1, p, T + 1, P, len, num_sec);
            else
                GenC2(next, t + 1, t, T + 1, P, len, num_sec);
            AddC(e);
            AddC(s);
        }
...
void GenC(int s, int t, int p, int v, int last)
{
    ...
    if (min == N * 2 && t % p == 0)
    {
        if (t == N)
            ...
            GenC2(head, 1, 1, t, N * 2 * p / t, v + 1, 0);
    }
